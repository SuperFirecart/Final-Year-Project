...
public class Node<GameObject> {
	...
	public void GetColliding(ref Bounds checkBounds, List<GameObject> result) {
		// Are the input bounds at least partially in this node?
		if (!bounds.Intersects(checkBounds)) {
			return;
		}
	...
	public void GetColliding(ref Ray checkRay, List<GameObject> result, float maxDistance = float.PositiveInfinity) {
		float distance;
		// Is the input ray at least partially in this node?
		if (!bounds.IntersectRay(checkRay, out distance) || distance > maxDistance) {
			return;
	...
	public void GetWithinFrustum(Plane[] planes, List<GameObject> result) {
	...
	public void SetChildren(Node<GameObject>[] childOctrees) {
	...
	public Node<GameObject> ShrinkIfPossible(float minLength) {
	...
	*/
	...
	/// <summary>
	/// Private counterpart to the public <see cref="Remove(GameObject, Bounds)"/> method.
	/// </summary>
	/// <param name="obj">Object to remove.</param>
	/// <param name="objBounds">3D bounding box around the object.</param>
	/// <returns>True if the object was removed successfully.</returns>
	...
	void Split() {
		...
		children = new Node<GameObject>[8];
		children[0] = new Node<GameObject>(newLength, minSize, looseness, Center + new Vector3(-quarter, quarter, -quarter));
		children[1] = new Node<GameObject>(newLength, minSize, looseness, Center + new Vector3(quarter, quarter, -quarter));
		children[2] = new Node<GameObject>(newLength, minSize, looseness, Center + new Vector3(-quarter, quarter, quarter));
		children[3] = new Node<GameObject>(newLength, minSize, looseness, Center + new Vector3(quarter, quarter, quarter));
		children[4] = new Node<GameObject>(newLength, minSize, looseness, Center + new Vector3(-quarter, -quarter, -quarter));
		children[5] = new Node<GameObject>(newLength, minSize, looseness, Center + new Vector3(quarter, -quarter, -quarter));
		children[6] = new Node<GameObject>(newLength, minSize, looseness, Center + new Vector3(-quarter, -quarter, quarter));
		children[7] = new Node<GameObject>(newLength, minSize, looseness, Center + new Vector3(quarter, -quarter, quarter));
	}
	...
	void Merge() {
		...
		for (int i = 0; i < 8; i++) {
			Node<GameObject> curChild = children[i];
	...
	bool ShouldMerge() {
		...
		if (children != null) {
			foreach (Node<GameObject> child in children) {
