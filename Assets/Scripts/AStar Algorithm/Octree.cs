// from https://github.com/Nition/UnityOctree
...
public class Octree<T> {
	...
	public Octree(float initialWorldSize, Vector3 initialWorldPos, float minNodeSize, float loosenessVal) {
		...
		rootNode = new Node<T>(initialSize, minSize, looseness, initialWorldPos);
	}
	...
	void Grow(Vector3 direction) {
		...
		rootNode = new Node<T>(newLength, minSize, looseness, newCenter);

		if (oldRoot.HasAnyObjects()) {
			// Create 7 new octree children to go with the old root as children of the new root
			int rootPos = rootNode.BestFitChild(oldRoot.Center);
			Node<T>[] children = new Node<T>[8];
			for (int i = 0; i < 8; i++) {
				if (i == rootPos) {
					children[i] = oldRoot;
				}
					...
					children[i] = new Node<T>(oldRoot.BaseLength, minSize, looseness, newCenter + new Vector3(xDirection * half, yDirection * half, zDirection * half));
				}
			}
